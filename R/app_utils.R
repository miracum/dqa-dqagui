...
render_difference_checks <- function(dat_table) {
  ...
  out <-
   DT::datatable(
     ...
     options = list(
       dom = "t",
       scrollY = "30vh",
       pageLength = nrow(dat_table$percent)
     ),
     rownames = FALSE
   ) %>%
  ...
  oldRow <- out[["x"]][["options"]][["rowCallback"]]

  # substitue for each column to get the percentage value

  newRow <- gsub("value=data\\[1\\];", "value; if (data[1] === \"no diff.\")
                 { value = 0; } else {
                 value = Math.abs(parseFloat(
                 data[1].match(/\\\\(([^)]+)\\\\)/)[1].replace('%', '')));}",
                 oldRow)
  newRow <- gsub("value=data\\[2\\];", "value; if (data[2] === \"no diff.\")
                 { value = 0; } else {
                 value = Math.abs(parseFloat(
                 data[2].match(/\\\\(([^)]+)\\\\)/)[1].replace('%', '')));}",
                 newRow)
  newRow <- gsub("value=data\\[3\\];", "value; if (data[3] === \"no diff.\")
                 { value = 0; } else {
                 value = Math.abs(parseFloat(
                 data[3].match(/\\\\(([^)]+)\\\\)/)[1].replace('%', '')));}",
                 newRow)
  newRow <- gsub("value=data\\[4\\];", "value; if (data[4] === \"no diff.\")
                 { value = 0; } else {
                 value = Math.abs(parseFloat(
